            ...
            <div className='bg-[#081028] border-b border-white border-opacity-5 pl-16 pr-4 py-4 flex items-center justify-between '>
                <div className='mb-4'>
                    <div className='flex items-center '>
                        <p className='text-inter text-white text-xl font-bold mt-5'>{api?.name}</p>
                        <div className="flex items-center justify-between pl-2 ml-2">
                            <div className="flex mt-6  py-0.5 px-1 bg-[#57C3FF] bg-opacity-30  border border-[#57C3FF] border-opacity-30 rounded-sm">
                ...
                <div className='flex '>
                    ...
                    <div className=' border-r-2 border-[#FFFFFF] border-opacity-5 px-10'>
                        <div className='flex mb-2 justify-center pb-2'>
                            <img src={voteIcon} />
                            <p className='  text-white text-opacity-60 pl-2 text-base'>Vote</p>
                        </div>
                        {api && (<Rating name="read-only" icon={<StarIcon style={{ color: '#FFFFFF', fontSize: 24 }} />} value={api?.votes} readOnly />)}
                    </div>
                    <div className=' border-r-2 border-[#FFFFFF] border-opacity-5 px-5'>
                        <div className='flex mb-2'>
                            <img src={latencyIcon} />
                            <p className=' text-white text-opacity-60 pl-2 text-base'> Latency</p>
                        </div>
                        {api && (<div className='text-white text-center text-lg'>{api?.latency}</div>)}
                    </div>
                    <div className=' border-r-2 border-[#FFFFFF] border-opacity-5 px-5'>
                        <div className='flex mb-2'>
                            <img src={voteIcon} />
                            <p className='  text-white text-opacity-60 pl-2 text-base'> Service Level</p>
                        </div>
                        {api && (<div className='text-white text-center text-lg'>{api?.service_level}%</div>)}
                    </div>
                    <div className='px-12'>
                        <div className='flex mb-2'>
                            <img src={plusIcon} />
                            <p className=' text-white text-opacity-60 pl-2 text-base'> Heath Check</p>
                        </div>
                        {api && (<div className='text-[#1F9E00] text-center text-lg'>{api?.health_check}</div>)}
                    </div>
